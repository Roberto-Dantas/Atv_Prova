...
function escritaIf(qT,nQ) {
    if(qT === nQ) {
        if(qts_naoR == 0) {
            document.querySelector('.parteIF').innerHTML = `<button class="backToInicio">Voltar ao Inicio</button></br></br></br></br><h2>Das ${questaoData.qntQ} questões, você acertou ${acertos} e finalizou em ${totalR} de ${questaoData.tempo}min</h2></br></br>`
        }else{
            document.querySelector('.parteIF').innerHTML = `<button class="backToInicio">Voltar ao Inicio</button></br></br></br></br><h2>Das ${questaoData.qntQ} questões, não respondeu ${qts_naoR}</h2><h2>Um total de ${acertos} acertos e não completou o questionário de ${questaoData.tempo}min</h2></br></br>`
        }
    }
    //else{
